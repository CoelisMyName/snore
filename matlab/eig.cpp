...
void eig(const double A[36], creal_T V[6])
{
  ...
  p = true;
  for (i = 0; i < 36; i++) {
    if ((!p) || (std::isinf(A[i]) || std::isnan(A[i]))) {
      p = false;
    }
  }
  if (!p) {
    ...
    if (p) {
      ...
      for (i = 0; i < 36; i++) {
        temp = A[i];
        T[i] = temp;
        if ((!p) || (std::isinf(temp) || std::isnan(temp))) {
          p = false;
        }
      }
      if (!p) {
        ...
        for (coltop = 0; coltop < 5; coltop++) {
          if (i <= 6) {
            std::memset(&T[(coltop * 6 + i) + -1], 0, (7 - i) * sizeof(double));
          }
          i++;
        }
      } else {
        ...
        for (b_i = 0; b_i < 5; b_i++) {
          ...
          if (temp != 0.0) {
            ...
            if (std::abs(beta1) < 1.0020841800044864E-292) {
              ...
              tau[b_i] = (beta1 - b_alpha1_tmp) / beta1;
