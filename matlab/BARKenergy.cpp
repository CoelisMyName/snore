...
void BARKenergy(const coder::array<double, 1U> &x, double fs, double BPSEE1[17],
    ...
    loop_ub = freq.size(1) - 1;
    for (i = 0; i <= loop_ub; i++) {
        freq[i] = freq[i] * fs / wlen;
    }
    ...
    for (b_loop_ub = 0; b_loop_ub < i; b_loop_ub++) {
        ...
        for (i1 = 0; i1 < loop_ub; i1++) {
            Sr[i1] = sourfft1[b_loop_ub + sourfft1.size(0) * i1];
        }
        // 取每一帧
        ...
        for (i1 = 0; i1 < 17; i1++) {
            BPS[b_loop_ub + BPS.size(0) * i1] = Src[i1];
            BPSE[b_loop_ub + BPSE.size(0) * i1] = Srt_en[i1];
        }
    }
    //  BPSE1= sum(BPSE,1);
    ...
    coder::sum(BPS, Srt_en);
    memcpy(&BPSEE1[0], &Srt_en[0], 17U * sizeof(double));
    // 求取每个子带的修正频谱质心
